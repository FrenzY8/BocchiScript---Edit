...
const start = (bocchi = new Client()) => {
    console.log(color(figlet.textSync('FrenzY X BOT 4.1', 'Larry 3D'), 'cyan'))
    console.log(color(figlet.textSync('Made By FrenzYSG', 'Larry 3D'), 'red'))
    console.log(color(figlet.textSync('Subscribe To FrenzY8', 'Larry 3D'), 'red'))
    ...
    bocchi.onMessage((message) => {
        bocchi.getAmountOfLoadedMessages()
            .then((msg) => {
                if (msg >= 10000000000000000000000) {
        ...
        try {
            if (event.action === 'add' && event.who !== botNumbers && isWelcome) {
                ...
                const welcomer = await new canvas.Welcome()
                    ...
                    .setGuildName(name)
                    .setAvatar(picx)
                    .setColor('border', '#FFD900')
                    .setColor('username-box', '#6A00FF')
                    .setColor('discriminator-box', '#00A6FF')
                    .setColor('message-box', '#D000FF')
                    .setColor('title', '#DDFF00')
                    .setBackground('https://www.photohdx.com/images/2016/05/red-blurry-background.jpg')
                    .toAttachment()
                const base64 = `data:image/png;base64,${welcomer.toBuffer().toString('base64')}`
                await bocchi.sendFile(event.chat, base64, 'welcome.png', `New Member!\n*Info*\n\nNama : *${pushname}*\nTag : *${pushname}*\nStatus : *Member Baru*\nGender :\n\nSelamat Datang Ya!\nFollow : *@kenajaga.exe* Biar Asik!`)
                ...
                await bocchi.sendFile(event.chat, base64, 'welcome.png', `Leave Member!\n*Info*\n\nNama : *${pushname}*\nTag : *${pushname}*\nStatus : *Member Baru*\nGender :\n\nSelamat Tinggal :(\nFollow : *@kenajaga.exe* Biar Ga Sedih`)
            }
        } catch (err) {
            console.error(err)
        }
